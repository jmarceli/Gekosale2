...
<div class="well well-small product-details review">
  <h2>{% trans %}TXT_OPINIONS{% endtrans %}</h2>
  <div class="alert alert-block alert-success" id="review-alert">
    ...
    <p><strong>{% trans %}TXT_HELP_OTHERS_BY_OPINION{% endtrans %}</strong></p>
    <p>{% trans %}TXT_YOUR_OPINION_HELP_A_LOT{% endtrans %}</p>
  </div>
  ...
  <form class="form-horizontal collapse" id="review" method="get" action="#">
    <fieldset>
      ...
      {% endfor %}
      <div class="control-group nomarginbt">
        <label class="control-label">{% trans %}TXT_OPINION_ABOUT_PRODUCT{% endtrans %}</label>
        <div class="controls">
      ...
      <div class="control-group">
        <label class="control-label">{% trans %}TXT_NAME_NICK{% endtrans %}</label>
        <div class="controls">
...
$(document).ready(function(){
	...
	$('#review').validate({
		...
		messages: {
			"nick":{
				"required":"{% trans %}TXT_NICK_REQUIRED{% endtrans %}",
			},
			"htmlopinion":{
