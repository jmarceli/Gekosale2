...
<div class="product-list row-fluid">
  ...
  {% else %}
  <p class="action"><a class="btn btn-danger" href="{{ path('frontend.contact', {"param": item.id }) }}"> {% trans %}TXT_PRODUCT_REQUEST{% endtrans %}</a></p>
  {% endif %}
  <a href="{{ path('frontend.productcart', {"param": item.seo}) }}" class="more">{% trans %}TXT_MORE_ABOUT_PRODUCT{% endtrans %} <i class="icon-arrow-right-blue"></i></a>
</div>
</div>
{% endfor %}
